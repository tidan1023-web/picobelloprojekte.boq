...
const EMPTY = { category: '', subCategory: '', item: '', unit: '', source: '', price: '', userAverage: '', currency: 'NGN' };
...
function PriceModal({ open, onClose, onSaved, editing }) {
  ...
  useEffect(() => {
    if (!open) return;
    setForm(editing
      ? { category: editing.category, subCategory: editing.subCategory ?? '', item: editing.item, unit: editing.unit, source: editing.source ?? '', price: editing.price, userAverage: editing.userAverage ?? '', currency: editing.currency ?? 'NGN' }
      : EMPTY);
    setError('');
  }, [open, editing]);
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1.5">My Average (optional)</label>
            <input type="number" min="0" step="0.01" value={form.userAverage} onChange={set('userAverage')} className={inputCls} placeholder="Your own benchmark average for this item" />
            <p className="text-xs text-gray-400 mt-1">Override the computed average with your own reference price.</p>
          </div>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row gap-3 mb-4 flex-wrap">
        ...
        {importMsg && (
          <span className="text-xs text-green-700 bg-green-50 border border-green-200 px-3 py-1.5 rounded-lg shrink-0">{importMsg}</span>
        )}
        {canEdit && (
          <>
            <ExcelImport
        ...
        return (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            {[
              ...
              { label: `Avg Price (${filtered.length})`,   value: avg,    color: 'bg-blue-50 border-blue-200 text-blue-800' },
              { label: 'Max Price',                        value: max,    color: 'bg-red-50 border-red-200 text-red-800' },
              { label: `My Avg (${withAvg.length} set)`,  value: myAvg,  color: 'bg-purple-50 border-purple-200 text-purple-800' },
            ].map(({ label, value, color }) => (
              <div key={label} className={`rounded-xl border p-4 ${color} ${value == null ? 'opacity-40' : ''}`}>
                <p className="text-xs font-semibold uppercase tracking-wide opacity-70 mb-1">{label}</p>
                <p className="text-lg font-bold">{value != null ? `${currency} ${fmt(value)}` : '—'}</p>
              </div>
            ))}
          </div>
        );
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
          <table className="w-full text-sm min-w-[640px]">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['Category', 'Sub-category', 'Item', 'Unit', 'Source', 'Price', 'My Avg', ''].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((p) => (
                <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-purple-700 font-semibold">{p.userAverage ? `${p.currency} ${Number(p.userAverage).toLocaleString()}` : '—'}</td>
