...
import {
  ...
  Calendar, DollarSign, Image, Pause, Play, BarChart3,
} from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="number" min={0} step="0.01" value={form.invoiceAmount} onChange={(e) => setForm((f) => ({ ...f, invoiceAmount: e.target.value }))} className={inputCls} placeholder="Amount invoiced / billed for this work" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Work Start Date</label>
              <input type="date" value={form.workStartDate} onChange={(e) => setForm((f) => ({ ...f, workStartDate: e.target.value }))} className={inputCls} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isPaused} onChange={(e) => setForm((f) => ({ ...f, isPaused: e.target.checked }))} className="rounded" />
              <span className="text-sm text-gray-700 font-medium flex items-center gap-1"><Pause size={13} /> Mark as Paused</span>
            </label>
          </div>
          {form.isPaused && (
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Pause Reason</label>
              <input value={form.pauseReason} onChange={(e) => setForm((f) => ({ ...f, pauseReason: e.target.value }))} className={inputCls} placeholder="e.g. Awaiting materials, Weather delay…" />
            </div>
          )}
          <div>
...
export default function ProgressTracker() {
  ...
  const selectedProject = projects.find((p) => p._id === selProjectId);
  ...
  const totalInvoiced  = updates.reduce((s, u) => s + (u.invoiceAmount || 0), 0);
  const latestCompletion = updates.length > 0 ? updates[0].completionPercent : 0;
  const budget         = selectedProject?.budget || 0;
  const remaining      = budget > 0 ? budget - totalActual : null;
  const budgetUsedPct  = budget > 0 ? (totalActual / budget) * 100 : null;
  const budgetPerformance = totalInvoiced > 0 ? totalInvoiced - totalActual : null;
  ...
  return (
    <div className="space-y-5">
      ...
      {selProjectId && updates.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <p className="text-xl font-bold text-gray-800">₦{fmt(totalInvoiced)}</p>
          </div>
          {remaining !== null ? (
            <div className={`rounded-2xl border shadow-sm p-4 ${remaining < 0 ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}>
              <div className="flex items-center gap-2 mb-1">
                <BarChart3 size={15} className={remaining < 0 ? 'text-red-500' : 'text-green-500'} />
                <p className="text-xs text-gray-500">Remaining Budget</p>
              </div>
              <p className={`text-xl font-bold ${remaining < 0 ? 'text-red-700' : 'text-green-700'}`}>₦{fmt(Math.abs(remaining))}</p>
              <p className="text-xs text-gray-500 mt-1">{budgetUsedPct?.toFixed(1)}% used</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
              <div className="flex items-center gap-2 mb-1">
      ...
      {budgetPerformance !== null && (
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-semibold ${budgetPerformance >= 0 ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
          <BarChart3 size={16} />
          Budget Performance: invoiced ₦{fmt(totalInvoiced)} vs actual spend ₦{fmt(totalActual)} —
          {budgetPerformance >= 0
            ? ` ₦${fmt(budgetPerformance)} under spend (on track)`
            : ` ₦${fmt(Math.abs(budgetPerformance))} over-invoiced vs actual`}
        </div>
      )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {u.pauseReason && <p className="text-xs text-yellow-600 mt-0.5">{u.pauseReason}</p>}
                      ...
                      <div className="flex flex-wrap items-center gap-4 mt-2 text-xs text-gray-400">
                        ...
                        {u.invoiceAmount > 0 && <span className="flex items-center gap-1 text-purple-600 font-medium"><DollarSign size={11} /> Inv: ₦{fmt(u.invoiceAmount)}</span>}
                        ...
                        {u.workStartDate && <span>Start: {new Date(u.workStartDate).toLocaleDateString('en-GB')}</span>}
                        {u.workEndDate && <span>End: {new Date(u.workEndDate).toLocaleDateString('en-GB')}</span>}
